...
export interface Plugin<Input extends PluginInput = {}> {
    /** Ran when the plugin is loaded by the ClairView plugin server. */
    setupPlugin?: (meta: Meta<Input>) => void
    /** Ran when the plugin is unloaded by the ClairView plugin server. */
    ...
    filterEvent?: (event: ClairViewEvent, meta: Meta<Input>) => boolean
    /** Not supported yet: Used for modifying events, return the updated event */
    modifyEvent?: (event: ClairViewEvent, meta: Meta<Input>) => ClairViewEvent
    ...
    onEventWithClairViewEvent?: (event: ClairViewEvent, meta: Meta<Input>) => Promise<void> | void
    /** Used for exporting a single event */
    composeWebhook?: (event: ClairViewEvent, meta: Meta<Input>) => Webhook | null
    /** @deprecated:  */
...
/** We're moving to a single ClairViewEvent model use this in any future apps/plugins 
*/
export interface ClairViewEvent {
    ...
    person?: ClairViewPerson
    groups?: Record<string, ClairViewGroup>
}

export interface ClairViewPerson {
    uuid: string
    properties: Properties
    clairview_url: string
}

export interface ClairViewGroup {
    key: string
    properties: Properties
    clairview_url: string 
}
...
/** @deprecated: Use ClairViewEvent.properties['$elements_chain'] */
...
// @deprecated
export interface ClairViewExtension {
